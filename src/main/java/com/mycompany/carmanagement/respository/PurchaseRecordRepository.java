...
public interface PurchaseRecordRepository extends PagingAndSortingRepository<PurchaseRecord, Long> {
	...
	@Query("select s from PurchaseRecord s where s.provider.id like :provider_id and s.beginDate >= :beginDate and s.endDate <= :endDate")
	List<PurchaseRecord> findByProviderId(@Param("provider_id") long provider_id, @Param("beginDate") Date beginDate, @Param("endDate") Date endDate);
	...
	@Query("select s from PurchaseRecord s where s.car.id like :car_id and s.provider.id like :provider_id and s.employee.id like :employee_id")
	List<PurchaseRecord> findWithoutID(@Param("car_id") long car_id, @Param("provider_id") long provider_id, @Param("employee_id") long employee_id);
